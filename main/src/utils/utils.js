...
function buildRoute(start, finish) {
    ...
    const URL = "https://router.project-osrm.org/route/v1/driving/" + `${startCoords[0]},${startCoords[1]};${finishCoords[0]},${finishCoords[1]}`;
    return fetch(URL)
    .then((r) => r.json())
    .then((data) => {
      const route = parseRoute(data.routes);
      const distance = (data.routes[0].distance / 1000).toFixed(2); // distance in kilometers
      const duration = new Date(data.routes[0].duration * 1000).toISOString().substr(11, 8); // duration in hh:mm:ss
      return { line: route, distance, duration };
    })
    .catch((e) => ({ line: null, distance: null, duration: null }));
}
